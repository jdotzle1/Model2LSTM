#!/usr/bin/env python3
"""
Weight Distribution Validation Script

Validates weight distributions per trading mode for the weighted labeling system.
Checks that weights are positive and analyzes distribution statistics.
...
from project.data_pipeline.validation_utils import WeightDistributionValidator


def main():
    parser = argparse.ArgumentParser(description='Validate weight distributions for weighted labeling system')
    ...
    if args.input_file:
        ...
        if input_file is None:
            ...
            for file_path in possible_files:
                print(f"  - {file_path}")
            print("\nUsage: python validate_weight_distributions.py <input_file.parquet>")
            sys.exit(1)
    ...
    try:
        ...
        # Run validation
        print("\nRunning weight distribution validation...")
        validator = WeightDistributionValidator()
        results = validator.validate_weight_distributions(df)
        
        # Print detailed report
        validator.print_weight_distribution_report(results)
            ...
            for mode_name, stats in results.items():
                ...
                for key, value in stats.items():
                    if hasattr(value, 'item'):  # numpy scalar
                        ...
                        # Handle nested dictionaries (like percentiles, winner_stats, loser_stats)
                        json_results[mode_name][key] = {}
                        for sub_key, sub_value in value.items():
                            if hasattr(sub_value, 'item'):
                                json_results[mode_name][key][sub_key] = sub_value.item()
                            else:
                                json_results[mode_name][key][sub_key] = sub_value
                    else:
